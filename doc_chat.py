...
MODEL = "qwen3.5:9b"
ENABLE_VISION = False                # set True to enable image upload (jpg, png, bmp)
...
ENABLE_INTENT_SCORING = False        # set True to enable LLM-based intent scoring + guardrails
ENABLE_ERROR_ALERTS = True          # send email to DEVOPS on chat errors
DEVOPS_EMAIL = "DEVOPS@efinance.com.eg" # recipient(s) for error alerts (comma-separated)
...
TEAM_PROMPTS: dict[str, list[dict[str, str]]] = {
    "QC": [
        {
            "label": "Get First 200 test cases",
            "prompt": (
                "You are an expert  software testing analysist Write software testing at least 200 test case for the attached document(s) for the positive and negative test case in excel table format with column  test case ID, requirement, Description, Pre‑conditions, Test Steps, expected results,"
            ),
        },
        {
            "label": "Get additional  200 test cases (to be used in case large document)",
            "prompt": (
                "You are an expert  software testing analysist check the attached document(s), and the attached test cases, then Write at least another additional 200 test case  for the remaining test case positive and negative test case in excel table format with column test case ID, requirement, Pre‑conditions, Test Steps, Description, expected results"
            ),
        },
        {
            "label": "Get the remining test cases",
            "prompt": (
                "You are an expert  software testing analysist check the attached document(s), and the attached test cases, then Write software testing for the remaining test case positive and negative test case in excel table format with column test case ID, requirement, Pre‑conditions, Test Steps, Description, expected results."
            ),
        },
    ],
}
...
- Use st.dataframe(df, width="stretch", hide_index=True) for clean tables.
...
def render_lobby():
    ...
    with col2:
        ...
        st.write("")
        if st.button("Start Chat", width="stretch", key="start_chat", type="primary"):
...
def render_toolbar():
    ...
    # Clear conversation
    with cols[col_idx]:
        if st.button("Clear Chat", width="stretch", key="chat_clear"):
    ...
    # Export
    with cols[col_idx]:
        if st.button("Export", width="stretch", key="chat_prepare_export"):
    ...
    # End session
    with cols[col_idx]:
        if st.button("End Session", width="stretch", key="chat_leave", type="primary"):
        ...
        for i, (name, kind) in enumerate(all_items):
            with rm_cols[i]:
                if st.button(f"✕ {name[:20]}", key=f"rm_{kind}_{name}",
                             width="stretch", help=f"Remove {name}"):
                    if kind == "doc":
...
def _render_queue_wait(position: int, prompt_id: str):
    ...
    with qc2:
        if st.button("Cancel", width="stretch", key="cancel_queue"):
    ...
    if available_prompts and not code_mode:
        ...
        for i, (label, sp_text) in enumerate(available_prompts):
            with sp_cols[i % len(sp_cols)]:
                if st.button(label, key=f"sp_{i}", width="stretch"):
...
def render_admin():
    ...
    with hc2:
        if st.button("Refresh", width="stretch", key="admin_refresh"):
            st.rerun()
    ...
    with qm_action_col:
        if total_in_queue > 0:
            if st.button("Clear All", key="admin_clear_queue", type="secondary",
                         width="stretch", help="Force-clear the entire queue"):
                prompt_clear_all()
                st.rerun()
