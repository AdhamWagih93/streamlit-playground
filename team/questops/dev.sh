...
# Add the host's DNS SEARCH domains to the container so short internal names
# resolve, WITHOUT touching the nameservers (replacing them would break the
# app->db compose-DNS). Written as a compose override — no env config needed.
gen_dns_override() {
  local override="$HERE/.dev/dns.yml" search=()
  for f in /run/systemd/resolve/resolv.conf /etc/resolv.conf; do
    [[ -r "$f" ]] || continue
    while read -r kw val _; do
      [[ "$kw" == search || "$kw" == domain ]] && for d in $val; do search+=("$d"); done
    done < "$f"
    [[ ${#search[@]} -gt 0 ]] && break
  done
  mkdir -p "$HERE/.dev"
  if [[ ${#search[@]} -eq 0 ]]; then rm -f "$override"; return; fi
  { echo "# AUTO-GENERATED by dev.sh — host DNS search domains. Do not edit."
    echo "services:"; echo "  app:"; echo "    dns_search:"
    for d in "${search[@]}"; do echo "      - \"$d\""; done
  } > "$override"
  COMPOSE+=(-f docker-compose.yml -f "$override")
  echo "adding host DNS search domains: ${search[*]}"
}
