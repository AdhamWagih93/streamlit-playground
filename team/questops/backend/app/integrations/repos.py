"""Shared local git workspaces for the configured repositories (REPO1..6).

Clones live server-side under REPOS_WORKDIR; edits stay LOCAL — nothing is
ever pushed from this page. Credentials are used only for clone/pull and
are never written into .git/config."""
...
def _demo_repos() -> list[dict]:
    return [{"slot": i + 1, "name": name,
             "url": f"https://git.example.local/platform/{name}.git",
             "user": "", "password": ""}
            for i, name in enumerate(DEMO_REPO_FILES)]
...
def _seed_demo_repo(repo: dict, repo_dir: Path) -> None:
    ...
    for rel, content in DEMO_REPO_FILES[repo["name"]].items():
        f = repo_dir / rel
